...
then
   ...
   do
       ...
       containerId=$(docker run -d -l SERVICE_tags=apihost=120.77.221.92,apiport=8083,weight=3 -p ${port}:${port} -v /logs/${image_name}/${port}:/log  --name="${image_name}_${port}" ${image_name}\
