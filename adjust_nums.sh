...
then
   ...
   do
       ...
       containerId=$(docker run -d -l SERVICE_tags=apihost=120.77.221.92,apiport=${port},weight=3 -p ${port}:${port} -v /root/logs/${image_name}/${port}:/logs  --name="${image_name}_${port}" ${image_name}\
       java -jar app.jar --logging.file=logs --spring.cloud.consul.host=120.77.221.92 --spring.cloud.consul.port=8500 --server.port=${port} --spring.cloud.consul.discovery.instance-id=${image_name})
