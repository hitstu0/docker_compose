...
then
   ...
   do
       ...
       containerId=$(docker run -d -l SERVICE_tags=apihost=120.77.221.92,apiport=${port},weight=3 -p ${port}:${port} -v /logs/${image_name}/${port}:/logs  --name="${image_name}_${port}" ${image_name}\
