...
then
   ...
   do
       ...
       containerId=$(docker run -d -l SERVICE_tags=apihost=$3,apiport=${port},weight=3 -p ${port}:${port} -v /root/logs/${image_name}/${port}:/logs  --name="${image_name}_${port}" ${image_name}\
       java -jar app.jar --service_name=$4 --logging.path=./logs --spring.cloud.consul.host=$3 --spring.cloud.consul.port=8500 --server.port=${port} --spring.cloud.consul.discovery.instance-id=${image_name})
